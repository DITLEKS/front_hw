...
describe('InputArea', () => {
  ...
  it('рендерится без ошибок', () => {
    render(<InputArea onSend={noop} isLoading={false} onStop={noop} />);
  ...
  it('при клике на «Отправить» вызывается onSend с текстом сообщения', async () => {
    ...
    render(<InputArea onSend={onSend} isLoading={false} onStop={noop} />);
    const textarea = screen.getByPlaceholderText('Введите сообщение...');
  ...
  it('в режиме isLoading показывается кнопка «Стоп»', () => {
    render(<InputArea onSend={noop} isLoading={true} onStop={noop} />);
  ...
  it('клик по «Стоп» вызывает onStop', async () => {
    ...
    render(<InputArea onSend={noop} isLoading={true} onStop={onStop} />);
